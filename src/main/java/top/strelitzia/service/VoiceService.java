...
public class VoiceService {
    ...
    public static String[] voiceList = new String[]{"任命助理", "交谈1", "交谈2", "交谈3", "晋升后交谈1", "晋升后交谈2", "信赖提升后交谈1", "信赖提升后交谈2", "信赖提升后交谈3", "闲置", "干员报到", "观看作战记录", "精英化晋升1", "精英化晋升2", "编入队伍", "任命队长", "行动出发", "行动开始", "选中干员1", "选中干员2", "部署1", "部署2", "作战中1", "作战中2", "作战中3", "作战中4", "完成高难行动", "3星结束行动", "非3星结束行动", "行动失败", "进驻设施", "戳一下", "信赖触摸", "标题", "问候"};

    @AngelinaGroup(keyWords = {"语音"}, description = "查询干员的某条语音", sort = "查询功能")
    public ReplayInfo getOperatorVoice(MessageInfo messageInfo) {
        ...
        if (args.size() > 1) {
            ...
            if (args.size() > 2) {
                ...
                return getReplayInfo(replayInfo, voices);
            } else {
    ...
    private ReplayInfo getReplayInfo(ReplayInfo replayInfo, List<String> voices) {
        if (voices.size() > 0) {
            File sourceFile = new File(voices.get(new Random().nextInt(voices.size())));//输入
            File targetFile = new File("runFile/Audio/source.mp3");//输出
            AudioUtil.wavToMp3(sourceFile, targetFile);//转换
            replayInfo.setReplayAudio(targetFile);
        }else {
            replayInfo.setReplayMessage("暂未找到结果，换个试试吧");
        }
        return replayInfo;
    }
}
