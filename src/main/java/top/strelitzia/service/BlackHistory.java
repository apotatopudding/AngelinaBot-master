...
/**
 * 此功能来自稀音bot，不加入琴柳正式包内，需要请自行下载使用
 */
@Service
public class BlackHistory {
    ...
    @AngelinaGroup(keyWords = {"黑历史","bh"}, description = "随机展现一张黑历史", sort = "娱乐功能", funcClass = "群黑历史")
    public ReplayInfo bh(MessageInfo messageInfo) {
        ...
        if (messageInfo.getArgs().size() > 1) {
            ...
            if(pic.exists()) {
                ...
                replayInfo.setReplayMessage("图片不存在，请输入正确的编号。可以通过“琴柳黑历史库”来查询当前图库编号范围");
            }
        }else {
    ...
    @SneakyThrows
    @AngelinaGroup(keyWords = {"黑历史库","bh库"}, description = "查看图库中的编号范围，或对图库进行快速编辑", sort = "娱乐功能", funcClass = "群黑历史")
    public ReplayInfo bhRange(MessageInfo messageInfo) {
        ...
        if (messageInfo.getArgs().size() > 1) {
                ...
                case "查空":
                    ...
                    /*case "管理":
                        replayInfo.setReplayMessage("目前图库共有”删除“、”添加“、”查空“三种功能"
                                + "\n琴柳图库 删除 编号，可以删除指定编号的图片，如果该图片非最后一张，将自动以最后一张填补其编号空缺"
                                + "\n琴柳图库 添加，可以将一张或多张图片添加至图库并自动编号"
                                + "\n琴柳图库 添加 编号，可以将一张图片添加至指定编号（请确保编号与图库中已有图片的编号连续，否则会出bug）"
                                + "\n琴柳图库 查空，可以快速发现空缺的编号（保险起见建议多查空几次）"
                                + "\n*如首次使用请继续发送“琴柳图库 注意事项”");
                        break;
                    case "注意事项":
            ...
            replayInfo.setReplayMessage("当前黑历史库共有" + num + "张图片"
                    + "\n请发送“琴柳黑历史 数字编号”来定向查找图片"
                    + "\n直接发送“琴柳黑历史”会随机抽取一张图片，并显示其id"
