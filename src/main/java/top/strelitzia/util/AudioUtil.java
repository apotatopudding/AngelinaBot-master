...
public class AudioUtil {
    public static void mp3ToAmr(File source,File target){
        ...
        //AMR-WB支持9种比特率，分别是6.6， 8.85， 12.65， 14.25， 15.85， 18.25， 19.85， 23.05， 23.85kbps，采样率为16KHz
    ...
    public static void wavToMp3(File source,File target){
        AudioAttributes audio = new AudioAttributes();
        audio.setCodec("libmp3lame");//编码器
        audio.setChannels(1);//声道；1单声道，2立体声

        EncodingAttributes attrs = new EncodingAttributes();
        attrs.setFormat("mp3");//格式
        attrs.setAudioAttributes(audio);//音频设置
        Encoder encoder = new Encoder();
        try {
            encoder.encode(source, target, attrs);
